...
def convert_to_cuda(optimizer):
    #utility to convert to cuda after loading from checkpoint
...
class SGLD(Optimizer):
    ...
    def step(self, burning_in=False, closure=None):
        ...
        for group in self.param_groups:
            ...
            for parameter in group["params"]:
                ...
                lr = group["lr"]
                num_pseudo_batches = self.defaults["num_pseudo_batches"]
                precondition_decay_rate = self.defaults["precondition_decay_rate"]
                gradient = parameter.grad.data

                #  State initialization {{{ #

                try:
                    #try to update iteration
                    state["iteration"]+=1
                except:
                    #assume first iteration if exception is raised
                    state["iteration"] = 0
                    state["momentum_buffer"] = torch.ones_like(parameter)
                ...
                #  Momentum update {{{ #
                momentum.add_((1.0 - precondition_decay_rate) *
                              ((gradient**2) - momentum))
                #  }}} Momentum update #
                ...
                preconditioner = (
                    1. / torch.sqrt(momentum + self.defaults["diagonal_bias"]))

                scaled_grad = (
                    0.5 * preconditioner * gradient * num_pseudo_batches +
                    torch.normal(mean=torch.zeros_like(gradient),
                                 std=torch.ones_like(gradient)) * sigma *
                    torch.sqrt(preconditioner))
